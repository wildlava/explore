...
100 LINE INPUT"Name of adventure: ";FI$
120 OPEN"I",1,FI$+".EXP"
130 LINE INPUT#1,AT$
...
170 PRINT CHR$(26):PRINT FI$;" is now being built..."
180 S=0:SL$(0)=","
...
210 IF NI$(F1,X1,Y1)="" OR LEFT$(NI$(F1,X1,Y1),1)="+" OR LEFT$(NI$(F1,X1,Y1),1)="-" OR TD$(F1,X1,Y1)="" THEN 270
...
370 SU$(S)=SU$(S)+S$+",":IF I2<>IN THEN 350  
380 NEXT
390 IF SU$(S)="," THEN SU$(S)=""
400 CLOSE
410 PRINT CHR$(26)
420 PRINT TAB(22) "*** The EXPLORE Adventure Series ***"
430 PRINT:M$=AT$:GOSUB 2682:PRINT
440 L=-1
450 PRINT"Do you want to resume a suspended game? (Y/N): ";
460 Y$=INKEY$:IF Y$<>"Y" AND Y$<>"y" AND Y$<>"N" AND Y$<>"n" THEN 460
470 IF Y$="N" OR Y$="n" THEN PRINT"No":GOTO 640 ELSE PRINT"Yes"
480 PRINT"Which suspend file? (0-9) --> ";
490 SF$=INKEY$:IF SF$="" THEN 490
500 IF ASC(SF$)<48 OR ASC(SF$)>57 THEN 490
510 PRINT SF$
...
590 NEXT:NEXT:NEXT:FOR U=0 TO 50:IF EOF(1) THEN 620 
600 LINE INPUT#1,UT$:IF UT$="+" THEN UT$(U)="+"
610 NEXT
620 CLOSE
630 L=L+1
640 D=1
650 IF TD$(F,X,Y)="" OR (NI$(F,X,Y)<>"" AND LEFT$(NI$(F,X,Y),1)<>"+" AND LEFT$(NI$(F,X,Y),1)<>"-") THEN 1530 
660 CI=0:IF NI$(F,X,Y)="" THEN 690
670 N$=RIGHT$(NI$(F,X,Y),LEN(NI$(F,X,Y))-1):FOR I=0 TO IL:IF I$(I)=N$ THEN IF LEFT$(NI$(F,X,Y),1)="-" GOTO 1530 ELSE GOTO 690  
680 NEXT:IF LEFT$(NI$(F,X,Y),1)="+" THEN 1530 
690 TD$=TD$(F,X,Y):TS=0:GOSUB 2830 :GOTO 1530 
700 IF LEFT$(TD$,1)<>"/" THEN 740  
710 I1=INSTR(TD$,","):I2=INSTR(I1+1,TD$,","):I3=INSTR(TD$,":")
720 IF I3=0 THEN I3=LEN(TD$)+1 ELSE PRINT:M$=RIGHT$(TD$,LEN(TD$)-I3):GOSUB 2682
730 X=VAL(MID$(TD$,2,I1-2)):Y=VAL(MID$(TD$,I1+1,I2-I1-1)):F=VAL(MID$(TD$,I2+1,I3-I2-1)):GOTO 630
740 IF LEFT$(TD$,1)<>"!" THEN 780  
750 IF L=0 THEN 1530 
760 PRINT:IF TD$<>"!" THEN M$=RIGHT$(TD$,LEN(TD$)-1):GOSUB 2682:PRINT
770 PRINT"It took you";L;"moves to win.":GOTO 2990 
780 IF LEFT$(TD$,1)=":" THEN M$=RIGHT$(TD$,LEN(TD$)-1):GOSUB 2682:RETURN
790 IF LEFT$(TD$,1)<>"=" THEN 840  
800 IF TD$="=" THEN RETURN
810 M$="":IN=INSTR(TD$,":"):IF IN=0 THEN IN=LEN(TD$)+1 ELSE M$=RIGHT$(TD$,LEN(TD$)-IN)
820 C$=MID$(TD$,2,IN-2):IF M$<>"" THEN GOSUB 2682
830 GOTO 1680 
840 IF LEFT$(TD$,1)<>"%" THEN 930  
850 I1=INSTR(TD$,","):IF I1=0 THEN RETURN
...
1000 TD$="+"
1010 IF M$<>"" THEN GOSUB 2682
1020 RETURN
1030 IF LEFT$(TD$,1)<>"-" THEN 1110  
1040 IF TD$="-" THEN RETURN
...
1090 IF M$<>"" THEN GOSUB 2682
1100 RETURN
1110 IF LEFT$(TD$,1)<>"#" THEN 1220 
...
1200 IF M$<>"" THEN GOSUB 2682
1210 RETURN
1220 IF LEFT$(TD$,1)<>"[" THEN 1430 
...
1400 TD$="["
1410 IF M$<>"" THEN GOSUB 2682
1420 RETURN
1430 IF LEFT$(TD$,1)<>"*" THEN 1500
1440 IF PT$(F,X,Y)="" THEN RETURN
...
1480 IF M$<>"" THEN GOSUB 2682
1490 RETURN
1500 PRINT:M$=TD$:GOSUB 2682:GOTO 2990 
1510 IF CI=1 THEN PRINT ER$
1520 RETURN
1530 IF D=0 THEN 1670
...
1580 IF (DE$(F,X,Y)<>"-" AND DE=1) OR (BL$(F,X,Y)<>"" AND BL=1) OR CN$(F,X,Y)<>"" THEN PRINT
1590 IF DE$(F,X,Y)<>"-" AND DE=1 THEN M$=DE$(F,X,Y):GOSUB 2682
1600 IF BL$(F,X,Y)<>"" AND BL=1 THEN M$=BL$(F,X,Y):GOSUB 2682
1610 IF CN$(F,X,Y)="" THEN 1670 
...
1720 NEXT
1730 IF NI$(F,X,Y)="" OR LEFT$(NI$(F,X,Y),1)="+" OR LEFT$(NI$(F,X,Y),1)="-" OR TD$(F,X,Y)="" THEN 1820 
...
1780 IF NI$="" THEN TS=0:GOTO 1810 
1790 FOR I=0 TO IL:IF I$(I)=NI$ THEN TS=0:GOTO 1810 
1800 NEXT:PRINT"You can't do that yet.":GOTO 1670 
1810 CI=1:GOSUB 2830:GOTO 650
1820 S=0
1830 IF SU$(S)="" THEN 2010 
1840 IF INSTR(SU$(S),","+C$+",")=0 THEN S=S+1:GOTO 1830 
1850 FOR U=0 TO 50
1860 IF UC$(U)="" THEN 2010 
1870 IF UC$="-" THEN 1980 
...
1910 IF C$<>MID$(UC$(U),I1,I2-I1) THEN IF I2=IN THEN 1980 ELSE GOTO 1900 
1920 IF UD$(U)="" THEN 1950
...
2100 IF C$<>"N" AND C$<>"S" AND C$<>"E" AND C$<>"W" AND C$<>"U" AND C$<>"D" THEN 2180 
...
2180 IF LEFT$(C$,4)<>"GET " THEN 2260 
2190 A$=RIGHT$(C$,LEN(C$)-4)
2200 GOSUB 2690 
2210 IF I1<>0 THEN 2230 
...
2260 IF LEFT$(C$,5)<>"DROP " THEN 2330
2270 A$=RIGHT$(C$,LEN(C$)-5):AS$=" "+A$+" "
2280 FOR I=0 TO IL
2290 I$=" "+I$(I)+" ":IF INSTR(I$,AS$)<>0 AND (INSTR(I$,AS$)=1 OR INSTR(I$,AS$)=LEN(I$)-LEN(AS$)+1) THEN 2320 
2300 NEXT
2310 PRINT"You are not carrying a ";A$;".":GOTO 1670 
2320 CN$=I$(I):GOSUB 2750 :IF CN$="" THEN PRINT"There is no place to put it.":GOTO 1670 ELSE I$(I)="":PRINT"Ok":GOTO 650
2330 IF INSTR(C$,"INVENT")=0 THEN 2420 
2340 FOR I=0 TO IL:IF I$(I)<>"" THEN 2370 
2350 NEXT
2360 PRINT"You are not carrying anything.":GOTO 1670 
2370 PRINT"You are currently holding the following:"
2380 PRINT
2390 FOR I=0 TO IL:IF I$(I)<>"" THEN PRINT"- ";I$(I);" -"
2400 NEXT
2410 PRINT:GOTO 1670 
2420 IF C$<>"SUSPEND" THEN 2590
2430 PRINT"Which suspend file? (0-9) --> ";
2440 SF$=INKEY$:IF SF$="" THEN 2440
2450 IF ASC(SF$)<48 OR ASC(SF$)>57 THEN 2440
2460 PRINT SF$
...
2510 IF RIGHT$(PT$(F1,X1,Y1),1)="+" THEN PRINT#1,"+" ELSE PRINT#1,""
...
2550 NEXT:NEXT:NEXT:FOR U=0 TO 50:IF UC$(U)="" THEN 2580 
2560 IF UT$(U)="+" THEN PRINT#1,"+" ELSE PRINT#1,""
2570 NEXT
2580 CLOSE:PRINT"Ok":GOTO 1670
2590 IF C$="LOOK" THEN 1540
2600 IF C$="QUIT" OR C$="STOP" THEN PRINT"Ok":GOTO 2990 
2610 IF C$<>"HELP" THEN 2660 
...
2650 GOTO 1670 
2660 S=0
2670 IF SL$(S)="" THEN PRINT"I don't understand.":GOTO 1670 
2680 IF INSTR(SL$(S),","+C$+",")=0 THEN S=S+1:GOTO 2670 ELSE PRINT"You can't do that here.":GOTO 1670 
2682 IN=LEN(M$)+1:I5=0
2684 I4=I5+1:I5=INSTR(I4,M$,"\"):IF I5=0 THEN I5=IN
2686 PRINT MID$(M$,I4,I5-I4)
2688 IF I5<>IN THEN 2684 ELSE RETURN 
2690 AS$=" "+A$+" ":IN=LEN(CN$(F,X,Y))+1
2700 I2=0
2710 I1=I2+1:I2=INSTR(I1,CN$(F,X,Y),","):IF I2=0 THEN I2=IN
2720 CN$=MID$(CN$(F,X,Y),I1,I2-I1):CS$=" "+CN$+" ":IF INSTR(CS$,AS$)<>0 AND (INSTR(CS$,AS$)=1 OR INSTR(CS$,AS$)=LEN(CS$)-LEN(AS$)+1) THEN 2740 
...
2890 IF I8<>I9 THEN 2850 
2900 IF TR$<>"+" AND TR$<>"[" THEN 2920 
...
2940 IF ERR=53 AND ERL=120 THEN CLOSE:PRINT"*** Adventure not found ***":RESUME 100
2950 IF ERR=53 AND ERL=520 THEN CLOSE:PRINT"*** Game not previously suspended ***":RESUME 450
2970 IF ERR=62 THEN CLOSE:PRINT CHR$(23):PRINT"Adventure name not valid":RESUME 100
